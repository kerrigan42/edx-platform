...
def generate_offer_data(user, course):
    ...
    if not can_receive_discount(user=user, course=course, discount_expiration_date=expiration_date):
        return None
...
def generate_offer_html(user, course):
    ...
    message_html = HTML(offer_message).format(
        a_open=HTML(u'<a id="welcome" href="{upgrade_link}">').format(upgrade_link=data['upgrade_url']),
        a_close=HTML('</a>'),
        b_open=HTML('<b>'),
        code=Text(data['code']),
        b_close=HTML('</b>'),
        ...
        discount_expiration_date=data['expiration_date'].strftime(u'%B %d'),
